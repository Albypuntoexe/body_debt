...
class SimulationRepository {
  ...
  DailyInput loadLogForDate(DateTime date) {
    ...
    if (history.containsKey(key)) {
      try { return DailyInput.fromMap(history[key]); } catch (e) { return const DailyInput(); }
    }
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // ALGORITMO ROBUSTO (Gestione Buchi Storici)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  int getProjectedWakeUpEnergy(UserProfile profile, DateTime targetDate) {
    ...
    // Calcolo debito storico
    double projectedDebt = 0.0;
    final history = _loadHistoryMap();

    for (int i = 7; i > 0; i--) {
      DateTime pastDate = targetDate.subtract(Duration(days: i));
      double slept = _getEffectiveSleep(pastDate, history, idealSleep, today);

      double diff = idealSleep - slept;
      if (diff > 0) projectedDebt += diff;
      else projectedDebt += (diff * 0.5);
    }

    if (projectedDebt < 0) projectedDebt = 0;
    double startEnergy = 100.0 - (projectedDebt * 5.0);
    return startEnergy.clamp(0, 100).toInt();
  }

  // Funzione Helper Intelligente per recuperare il sonno
  double _getEffectiveSleep(DateTime date, Map<String, dynamic> history, double ideal, DateTime today) {
    // Se è futuro rispetto a oggi: Neutro
    if (date.isAfter(today)) return ideal;

    String key = _dateToKey(date);

    // Se non c'è nel log: Neutro (Assumiamo che l'utente abbia dormito ma non usato l'app)
    if (!history.containsKey(key)) return ideal;

    // Se c'è nel log:
    double loggedSleep = DailyInput.fromMap(history[key]).sleepHours;

    // FIX CRITICO: Se trovo 0 ore registrate in un giorno PASSATO (non oggi),
    // assumo che sia un errore di logging (dimenticanza) e non una notte in bianco.
    // Questo evita il crollo dell'energia se salti un giorno.
    if (loggedSleep == 0 && date.isBefore(today)) {
      return ideal;
    }
  ...
  SimulationResult runSimulation(UserProfile profile, DailyInput currentInput, DateTime targetDate, {bool isForecast = false}) {
    ...
    // Calcoliamo il debito basandoci sui giorni precedenti
    for (int i = 3; i > 0; i--) {
      DateTime pastDate = targetDate.subtract(Duration(days: i));
      double slept = _getEffectiveSleep(pastDate, history, idealSleep, todayDate);
    ...
    if (isFuture) {
      ...
      List<int>? wakeParts = _safeParseTime(currentInput.wakeTimeStr);
      List<int>? bedParts = _safeParseTime(currentInput.bedTimeStr);

      if (wakeParts != null && bedParts != null) {
        wakeUpTime = DateTime(targetDate.year, targetDate.month, targetDate.day, wakeParts[0], wakeParts[1]);
        double bedDouble = bedParts[0] + (bedParts[1] / 60.0);
        double wakeDouble = wakeParts[0] + (wakeParts[1] / 60.0);
    ...
    if (isToday) {
      // Se usePrecise è false, assumiamo sveglia dinamica o default 8:00
      // Usiamo l'ora di sveglia definita sopra
  ...
  List<int>? _safeParseTime(String? timeStr) {
